...
const OptionSelectCity = () => {
    ...
    const filteredArray = cities.filter(item1 =>
    ...
    return (
        <div ref={selectRef} className="relative w-full h-full mx-auto">
            <div
                onClick={() => {
                    openOption ? setOpenOption(false) : setOpenOption(true); cities.map((city) => {
            ...
            <ul className={`z-40 absolute top-9 w-full ${openOption ? 'max-h-[200px] block' : "h-0 hidden"} mt-2 rounded bg-white shadow-[0px_1px_3px_1px_#00000026] overflow-x-hidden overflow-y-scroll`}>
                ...
                {filteredArray.map((city, index) => (
                    <div
                        ...
                        ${city.name.toLowerCase().includes(valueInput) ? "block rounded-lg px-4 overflow-x-hidden" : "hidden"} 
                    ...
                    >
                        ...
                        <div className="text-[#161719] font-normal text-sm leading-4">{city.name}</div>
                    </div>
                ))}
            </ul>
        </div>
